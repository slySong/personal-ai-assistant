...
- **完全本地运行**：基于 Ollama + gpt-oss，数据不出本机，零 API 费用
...
从 https://ollama.com/download 下载 Windows 版，双击安装。

- 安装后会自动加入 PATH，并开机自启一个后台服务监听 `http://localhost:11434`
- 验证：打开 PowerShell 执行
  ```powershell
  ollama --version
  ```
...
```powershell
# 生产模型（约 12GB，需要 16GB+ 内存或 8GB+ VRAM 的 GPU）
ollama pull gpt-oss:20b

# 开发期快速迭代用（约 5GB，CPU 也能流畅跑，推荐先用这个调试）
ollama pull qwen3:8b
```

> **硬件建议**：
> - 最低：16GB 内存，gpt-oss:20b 纯 CPU 约 2-3 tok/s（较慢）
> - 推荐：NVIDIA GPU 8GB+ VRAM，可流畅跑 20b
> - 无 GPU 时强烈建议开发期用 qwen3:8b
...
1. 启动后，状态栏应显示绿色 "● Ollama 已连接"。若显示红色，点 **设置** 检查 Ollama 是否运行。
2. 在输入框输入消息，Ctrl+Enter 发送。
3. 试着告诉助手你的偏好，例如：
   - "我叫小张，我是做前端开发的"
   - "我主要用 TypeScript 写代码"
   - "回复尽量简洁，代码块带语言标注"
4. 助手会自动记住这些偏好。打开右侧 **记忆面板** 可查看/编辑已记录的偏好。
5. 下次对话时，助手会自动应用这些偏好。
...
本项目在 Windows 上采用**防御性限制**而非真隔离：

- **工作目录禁锢**：文件操作限制在 `data/sandbox/` 内，拒绝 `..` 和绝对路径越界
...
├── core/
│   ├── agent.py             # Agent 循环（tool calling + CoT 回传）
│   ├── llm_client.py        # Ollama/gpt-oss 封装，流式 tool_calls 累积
│   ├── memory.py            # 偏好记忆（规则+LLM 提取，注入 system prompt）
│   └── conversation.py      # 对话历史 + 滑动窗口裁剪
├── tools/
│   ├── base.py              # Tool 抽象基类 + ToolRegistry
│   ├── file_ops.py          # 文件读写（路径禁锢）
...
### Q: 启动后状态栏显示 "Ollama 未连接"
- 检查 Ollama 服务是否运行：任务管理器看是否有 `ollama.exe` 进程
- 手动启动：PowerShell 执行 `ollama serve`
- 检查端口：`curl http://localhost:11434/api/tags`

### Q: 模型响应很慢
- gpt-oss:20b 在纯 CPU 上约 2-3 tok/s，属正常
- 切换到更快的模型：设置里改成 `qwen3:8b`
- 确认是否有 GPU 加速：`ollama ps` 查看 GPU 使用情况

### Q: 工具调用失败
- 查看 GUI 中工具调用过程的错误提示
- 联网搜索失败可能是 DuckDuckGo 限流，稍后重试
- 文件操作失败检查路径是否在 sandbox 内
